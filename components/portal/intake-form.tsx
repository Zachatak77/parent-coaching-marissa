  ...
  if (done) {
    return (
      <div className="bg-white rounded-2xl border border-[#D9CFB9] p-10 shadow-sm text-center space-y-4">
        ...
        <Link
          href="/portal"
          className="inline-flex items-center gap-2 mt-2 px-5 py-2.5 rounded-lg bg-[#5F728D] text-white text-sm font-semibold hover:bg-[#54647C] transition-colors"
        >
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Section title="Section 1: About Your Family">
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Parent name(s)" required error={errors.parent_name?.message}>
            <Input {...register('parent_name')} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
          </Field>
          <Field label="Phone number" error={errors.phone?.message}>
            <Input {...register('phone')} type="tel" className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
          </Field>
        </div>
      ...
      <Section title="Section 2: What Brings You Here">
        <Field label="What is the main challenge you're facing right now?" required error={errors.main_challenge?.message}>
          <Textarea {...register('main_challenge')} rows={3} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        ...
        <Field label="Have you tried any strategies so far? If so, what?" error={errors.strategies_tried?.message}>
          <Textarea {...register('strategies_tried')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="What does success look like for your family?" required error={errors.success_looks_like?.message}>
          <Textarea {...register('success_looks_like')} rows={3} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
      </Section>
      ...
      <Section title="Section 3: Your Child">
        <Field label="What words would you use to describe your child's temperament?" error={errors.child_temperament?.message}>
          <Textarea {...register('child_temperament')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="Are there any diagnoses or evaluations we should be aware of? (optional)" error={errors.diagnoses?.message}>
          <Textarea {...register('diagnoses')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="What does your child respond well to?" error={errors.responds_well?.message}>
          <Textarea {...register('responds_well')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="What tends to escalate situations?" error={errors.escalates?.message}>
          <Textarea {...register('escalates')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
      </Section>
      ...
      <Section title="Section 4: Your Parenting Style">
        <Field label="How would you describe your current parenting approach?" error={errors.parenting_approach?.message}>
          <Textarea {...register('parenting_approach')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="What are your biggest strengths as a parent?" error={errors.strengths?.message}>
          <Textarea {...register('strengths')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
        <Field label="Where do you feel you need the most support?" error={errors.needs_support?.message}>
          <Textarea {...register('needs_support')} rows={2} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
      </Section>
      ...
      <Section title="Section 5: Logistics">
        ...
        <Field label="Anything else you'd like your coach to know? (optional)" error={errors.anything_else?.message}>
          <Textarea {...register('anything_else')} rows={3} className="border-[#D9CFB9] focus-visible:ring-[#5F728D]" />
        </Field>
      </Section>
      ...
      <button
        ...
        className="w-full py-3 rounded-lg bg-[#5F728D] text-white font-semibold text-sm hover:bg-[#54647C] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
      >
